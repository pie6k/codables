...
Use the `keys` array to specify exactly which properties to encode:
...
class SelectiveType {
  ...
  constructor(data: Pick<SelectiveType, "name" | "email" | "password" | "internalId">) {
    this.name = data.name;
...
const coder = new Coder([SelectiveType]);
const instance = new SelectiveType({
  name: "John",
...
// { $$SelectiveType: [{ name: "John", email: "john@example.com" }] }
// password and internalId are not included
```


<Callout type="info">
Key mapping works in both directions - during encoding, properties are mapped to the specified keys, and during decoding, the keys are mapped back to the original property names.
</Callout>